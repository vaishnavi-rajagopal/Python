...
def abort_add(root):
	for i in range(0,len(root)):
		if root[i].tag=='actions':
			abortAction=ET.SubElement(root[i],'abortAction')
			for j in range(0,len(root[i])):
				if root[i][j].tag=='abortAction':
					if len(root[i][j])==0:
						status=ET.SubElement(root[i][j],'status')
						for k in range(0,len(root[i][j])):
							root[i][j][k].text='Failed'
	return root;
...
def main():
	...
	for t in range(0,depth_main):
		...
		if v_action.lower() in ('update'):

			backup(tgt_url,obj_name)

			res = requests.get(tgt_url, auth=('vaverma', passwd))
			with open('data.xml', 'wb') as file:
				file.write(res.content)
			
			tree = ET.parse('data.xml')
			root = tree.getroot()
			param=ET.tostring((abort_add(root)), encoding='utf-8', method='xml')

			res_upd = requests.put(tgt_url, auth=('vaverma', passwd), data = param, headers=header)
			print res_upd.text
